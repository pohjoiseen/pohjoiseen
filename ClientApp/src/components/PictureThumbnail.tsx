 ...
 * @param link  Display image in <a> element with specified href
...
const PictureThumbnail = ({ picture, isSelected, link, onOpen, onClick, onRetryUpload, isError, isLoading }: PictureThumbnailProps) => {
    ...
    if (link) {
        return <a href={link} className="display-block me-2 mb-2 position-relative picture-thumbnail" title={title}>{pictureElem}</a>;
    }

    return <div className="me-2 mb-2 position-relative picture-thumbnail" title={title}>{pictureElem}</div>
};

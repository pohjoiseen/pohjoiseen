...
[`src/index.js`](src/index.tsx) first by modifying the service worker import:
```javascript
...
The last command shown above will serve your static site on the port **5069**. Like many of [serve](https://github.com/zeit/serve)’s internal settings, the port can be adjusted using the `-p` or `--port` flags.

...
{
    private IStringLocalizer<Fennica> _localizer;
        
    public BlogController(IStringLocalizer<Fennica> localizer)
    {
    ...
    {
        int num = posts.Count() / Fennica.PostsPerPage;
        if (posts.Count() % Fennica.PostsPerPage > 0)
        {
            num++;
        }
        return num;
    ...
    public async Task RenderRSS(Blog blog, HttpResponse response)
    {
        ...
        if (posts.Count > Fennica.PostsPerPage)
        {
            posts = posts.Take(Fennica.PostsPerPage).ToList();
        }
            
        // init feed
        var feed = new SyndicationFeed(Fennica.MainTitle, _localizer["rss-description"],
            new Uri($"{Fennica.PublicBase}/{blog.Language}/"));
        feed.Generator = "Fennica2/Teos";
        feed.Language = blog.Language;
        feed.Copyright = new TextSyndicationContent(Fennica.Copyright + " " + Fennica.Author);
        ...
        foreach (var post in posts)
        {
            var item = new SyndicationItem(post.Title, post.Description,
                new Uri(Fennica.PublicBase + post.CanonicalURL),
                Fennica.PublicBase + post.CanonicalURL,
            ...
            {
                var link = SyndicationLink.CreateMediaEnclosureLink(new Uri(Fennica.PublicBase + post.TitleImage),
                    "image/jpeg", 0);
                item.Links.Add(link);
            }
    ...
    public override async Task Render(Content content, GroupCollection routeMatch, HttpResponse response)
    {
        ...
        var start = (page - 1) * Fennica.PostsPerPage;
        if (start >= 0 && start < allPosts.Count)
        {
            var count = Fennica.PostsPerPage;
